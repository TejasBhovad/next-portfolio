...
export function Model(props) {
  ...
  // amek default animation of model to action[names[1]]
  useEffect(() => {
    if (!clicked) {
      actions[names[1]].setLoop(LoopRepeat).fadeIn(0.7).play();
    }
  }, []);
  ...
  useEffect(() => {
    if (clicked) {
      actions[names[1]].setLoop(LoopRepeat).fadeIn(0.25).stop();
      actions[names[0]].setLoop(LoopOnce, 1).fadeIn(0.7).play().reset();
      actions[names[0]].play();
      click(false);
      actions[names[1]].setLoop(LoopRepeat).fadeIn(4.75).play();
    }
  }, [clicked]);
  // play the actions[1] infinitely
